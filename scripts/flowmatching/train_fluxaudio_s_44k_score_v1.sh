#!/usr/bin/env bash
# FluxAudio-S 44kHz + Score Conditioning v1 (GlobalAdaLN, global_c add)
# Native 44kHz generation — latent_dim=40, seq_len=430
# Requires 44k NPZ data (extract_audio_latents_44k.py)
#
# This is the Stage-1 SFT script for the deployed submission chain:
# Stage 1 (v1 GlobalAdaLN, this script) -> Stage 2 (v1, expert iter)
#  -> Stage 3 (cross-load v1 weights into v2 InputAdd forward, CRPO).
# See README "Training pipeline" and the paper §III for the rationale
# (v1 is the more stable SFT variant under our budget).
...
text_c_dim=512
model=fluxaudio_score_s_v1_44k
null_score_prob=0.1
